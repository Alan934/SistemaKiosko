...
namespace StockKiosko
{
    public partial class frmProveedores : Form
    {
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            {
                int idGenerado = new CNProveedor().Registrar(obj, out Mensaje);

                if (idGenerado != 0)
                {
                    dvgData.Rows.Add(new object[] {"", idGenerado, txtDocumento.Text, txtRazonSocial.Text, txtCorreo.Text, txtTelefono.Text,
            ...
            {
                bool resultado = new CNProveedor().Editar(obj, out Mensaje);
                if (resultado)
                {
        ...
        private void dvgData_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dvgData.Columns[e.ColumnIndex].Name == "btnSeleccionar")
            {
                ...
                if (indice >= 0)
                {
                    ...
                    txtRazonSocial.Text = dvgData.Rows[indice].Cells["RazonSocial"].Value.ToString();
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MessageBox.Show("¿Desea Borrar este Proveedor?", "Mensaje", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    string Mensaje = string.Empty;
                    Proveedor obj = new Proveedor()
                    {
